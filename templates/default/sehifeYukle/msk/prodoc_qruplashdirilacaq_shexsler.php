        ...
        <div class="col-md-6">
            <div class="table-responsive">
                <table class="table table-striped table-bordered table-advance table-hover" id="shobeler_table">
                    ...
                    <tbody>
                    <?php
                    $shobelerSiyahi = DB::fetchAll("SELECT * FROM tb_prodoc_group WHERE is_deleted='0'");
                    ...
                    foreach($shobelerSiyahi as $shobeInfo)
                    {
                        ...
                        print '<td>'.$sira.'</td>';
                        print '<td>'.htmlspecialchars($shobeInfo['name']).'</td>';
...
<script>
    ...
    $('#shobeler_table>tbody').on("click",".blue",function()
    {
        ...
        {
            $.post(proBundle + "includes/msk/qruplashdirilacaq_shexsler.php", {'shobe_ad':shobe_ad,'shobe_id':tr_id,'ds':ds,'mm':mm,'dr':dr},function(result)
            {
                if(result=="error")
                {
    ...
    $('#shobeler_table>tbody').on("click",".purple",function()
    {
        ...
        $(this).parent('td').parent('tr').html("<td style='width:20px;text-align:center;'>"+sayi+"</td><td><input type='text' class='form-control' value='"+adi+"'></td><td style='text-align:center;'><a href='javascript:;' class='btn default btn-xs blue'><i class='fa fa-save'></i> Yadda saxla</a></td><td style='width:100px;text-align:center;'><a href='javascript:;' class='btn default btn-xs yellow'><i class='fa fa-remove'></i> İmtina</a></td>");
        th.find('input[type="checkbox"]').uniform();
        th.find('.yellow').click(function()
        {
            th.html("<td style='width:20px;text-align:center;'>"+sayi+"</td><td>"+adi+"</td><td style='text-align:center;'><a href='javascript:;' class='btn default btn-xs purple'><i class='fa fa-edit'></i> Düzəliş</a></td><td style='width:100px;text-align:center;'><a href='javascript:;' class='btn default btn-xs red'><i class='fa fa-trash'></i> Sil</a></td>");
        });
    });
    $('#shobeler_table>tbody').on("click",".red",function()
    {
        ...
        $('#basic [data-id=beli]').unbind('click').click(function()
        {
            ...
            $.post(proBundle + "includes/msk/qruplashdirilacaq_shexsler.php", {'shobe_id':idsi,'shobe_sil':"shobe_sil"},function(result)
            {
                $('#basic .default').click();
    ...
    $('#shobeler_table>tbody').on("click","tr",function()
    {
        ...
        $.post(proBundle + "includes/msk/group_user.php",{'shobe':$(this).attr("tr_id")},function(netice)
        {
    ...
    $('#shobe_emekdash_table>tbody').on("click",".blue",function()
    {
        ...
        {
            $.post(proBundle + "includes/msk/qruplashdirilacaq_shexsler.php", {'shobe_emekdash_id':shobe_emekdash_id,'shobe_id':$('#shobeler_table>tbody>tr.sechilib').attr("tr_id")},function(result)
            {
                if(result=="error")
                {
                    tr.after('<tr error><td colspan="100%" style="color:red;font-size:11px;">Bu adla şöbə mövcuddur.</td></tr>');
    ...
    $('#shobe_emekdash_table>tbody').on("click",".red",function()
    {
        ...
        {
            var idsi = $('#basic').attr('idsi');
            $.post(proBundle + "includes/msk/qruplashdirilacaq_shexsler.php", {'shobe_emekdash_id':idsi,'shobe_emekdash_sil':"shobe_emekdash_sil"},function(result)
            {
                $('#basic .default').click();
                t.parent('td').parent('tr').remove();
